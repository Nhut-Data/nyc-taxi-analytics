...
from spark_jobs.validations.rules import (
    is_valid_fare, is_valid_distance, is_valid_duration,
    is_valid_location_id, is_not_null_datetime, is_all_valid, get_reason_code,
    REASON_NEGATIVE_FARE, REASON_SUSPICIOUS_DISTANCE,
    REASON_INVALID_DURATION, REASON_INVALID_LOCATION_ID, REASON_NULL_DATETIME,
)
from spark_jobs.jobs.conform_trips import normalize_columns, cast_types, fill_optional_cols
from spark_jobs.schemas.trip_schema import COLUMN_RENAME_MAP
...
class TestIsValidFare:
    def test_positive_fare_is_valid(self, spark):
        df = make_trip_df(spark, [
            {"fare_amount": 10.5, "trip_distance": 2.0,
             "pickup_datetime": None, "dropoff_datetime": None,
             "pu_location_id": 1, "do_location_id": 2}
        ])
        assert df.filter(is_valid_fare()).count() == 1

    def test_zero_fare_is_valid(self, spark):
        df = make_trip_df(spark, [
            {"fare_amount": 0.0, "trip_distance": 0.0,
             "pickup_datetime": None, "dropoff_datetime": None,
             "pu_location_id": 1, "do_location_id": 2}
        ])
        assert df.filter(is_valid_fare()).count() == 1
    ...
    def test_negative_fare_is_invalid(self, spark):
        df = make_trip_df(spark, [
            {"fare_amount": -5.0, "trip_distance": 2.0,
             "pickup_datetime": None, "dropoff_datetime": None,
             "pu_location_id": 1, "do_location_id": 2}
        ])
...
class TestIsValidDistance:
    def test_normal_distance_is_valid(self, spark):
        df = make_trip_df(spark, [
            {"fare_amount": 10.0, "trip_distance": 5.0,
             "pickup_datetime": None, "dropoff_datetime": None,
             "pu_location_id": 1, "do_location_id": 2}
        ])
    ...
    def test_zero_distance_is_valid(self, spark):
        df = make_trip_df(spark, [
            {"fare_amount": 2.5, "trip_distance": 0.0,
             "pickup_datetime": None, "dropoff_datetime": None,
             "pu_location_id": 1, "do_location_id": 2}
        ])
        assert df.filter(is_valid_distance()).count() == 1
    ...
    def test_extreme_distance_is_invalid(self, spark):
        df = make_trip_df(spark, [
            {"fare_amount": 10.0, "trip_distance": 312722.0,
             "pickup_datetime": None, "dropoff_datetime": None,
             "pu_location_id": 1, "do_location_id": 2}
        ])
        assert df.filter(is_valid_distance()).count() == 0
...
class TestIsValidLocationId:
    def test_valid_location_ids(self, spark):
        df = make_trip_df(spark, [
            {"fare_amount": 10.0, "trip_distance": 2.0,
             "pickup_datetime": None, "dropoff_datetime": None,
             "pu_location_id": 132, "do_location_id": 236}
        ])
    ...
    def test_location_id_out_of_range(self, spark):
        df = make_trip_df(spark, [
            {"fare_amount": 10.0, "trip_distance": 2.0,
             "pickup_datetime": None, "dropoff_datetime": None,
             "pu_location_id": 999, "do_location_id": 1}
        ])
    ...
    def test_location_id_zero_is_invalid(self, spark):
        df = make_trip_df(spark, [
            {"fare_amount": 10.0, "trip_distance": 2.0,
             "pickup_datetime": None, "dropoff_datetime": None,
             "pu_location_id": 0, "do_location_id": 1}
        ])
        assert df.filter(is_valid_location_id()).count() == 0
...
class TestReasonCode:
    ...
    def test_extreme_distance_gets_correct_reason(self, spark):
        df = make_trip_df(spark, [
            {"fare_amount": 10.0, "trip_distance": 99999.0,
             "pickup_datetime": ts("2024-01-01 08:00:00"),
             "dropoff_datetime": ts("2024-01-01 08:15:00"),
             "pu_location_id": 1, "do_location_id": 2}
        ])
