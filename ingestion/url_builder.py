...
URL pattern đã verify: https://d37ci6vzurychx.cloudfront.net/trip-data/{service_type}_tripdata_{YYYY}-{MM}.parquet
"""
...
def build_gcs_path(year: int, month: int, service_type: str, bucket: str) -> str:
    ...
    """
    return f"gs://{bucket}/trip-data/{service_type}/{year}/{month:02d}/{service_type}_tripdata_{year}-{month:02d}.parquet"


if __name__ == "__main__":
    # Quick smoke test
    url = build_trip_url(2024, 1, "yellow")
    gcs = build_gcs_path(2024, 1, "yellow", "nyc-taxi-raw-banking-500108")
    print(f"URL : {url}")
    print(f"GCS : {gcs}")